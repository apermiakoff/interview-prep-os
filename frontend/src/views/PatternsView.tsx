    ...
    <main className="view page-shell" id="main-content">
      ...
      <section className="pattern-list">
        {data.patterns.map((pattern, index) => <article className="pattern-row" key={pattern.id}>
          ...
          <aside><span>Evidence</span><strong>{pattern.evidence_count}</strong><span>Independent</span><strong>{pattern.independent_count}</strong><span>Confidence</span><strong>{pattern.confidence}</strong><button className="text-link button-link" onClick={() => navigate("lab")}>Open visual lesson →</button></aside>
        </article>)}
      </section>
